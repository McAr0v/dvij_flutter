...
class PromotionsFeedPage extends StatefulWidget {
  ...
  @override
  _PromotionsFeedPageState createState() => _PromotionsFeedPageState();
}



class _PromotionsFeedPageState extends State<PromotionsFeedPage> {
  ...
  Future<void> _initializeData() async {
    ...
    if (PromoCustom.currentFeedPromoList.isEmpty){
      // ---- Если список пуст ----
      // ---- И Юзер залогинен
      // ---- Считываем с БД заведения -----

      if (UserCustom.currentUser?.uid != null && UserCustom.currentUser?.uid != ''){
        tempPromosList = await PromoCustom.getAllPromos();
      }
      ...
      tempPromosList = PromoCustom.currentFeedPromoList;
    ...
    return Scaffold(
      body: RefreshIndicator (
        ...
        onRefresh: () async {
          ...
          List<PromoCustom> tempEventsList = await PromoCustom.getAllPromos();
